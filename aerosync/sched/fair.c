...
static void throttle_cfs_rq(struct cfs_rq* cfs_rq) {
  ...
  struct sched_entity* se = cfs_rq->cs ? cfs_rq->cs->se[rq->cpu] : nullptr;

  if (cfs_rq->throttled) return;

  cfs_rq->throttled = 1;

  if (se && se->on_rq) {
    dequeue_task_fair(rq, container_of(se, struct task_struct, se), DEQUEUE_SAVE);
  }

  /* Add to throttled list in bandwidth control */
  if (cfs_rq->cs) {
    spinlock_lock(&cfs_rq->cs->bandwidth.lock);
    list_add_tail(&cfs_rq->throttled_list, &cfs_rq->cs->bandwidth.throttled_cfs_rq);
    spinlock_unlock(&cfs_rq->cs->bandwidth.lock);
  }
}
...
static void unthrottle_cfs_rq(struct cfs_rq* cfs_rq) {
  ...
  if (se && !se->on_rq) {
    enqueue_task_fair(rq, container_of(se, struct task_struct, se), ENQUEUE_RESTORE);
  }
}

/*
 * Period timer callback for CPU bandwidth control
 */
void sched_cfs_period_timer(struct timer_list* timer) {
  struct cpu_rd_state* cs = container_of(timer, struct cpu_rd_state, bandwidth.period_timer);
  struct cfs_rq *cfs_rq, *tmp;

  spinlock_lock(&cs->bandwidth.lock);
  cs->bandwidth.runtime = cs->bandwidth.quota;

  /* Unthrottle all runqueues in this domain */
  list_for_each_entry_safe(cfs_rq, tmp, &cs->bandwidth.throttled_cfs_rq, throttled_list) {
    unthrottle_cfs_rq(cfs_rq);
  }

  /* Restart timer */
  timer_add(timer, get_time_ns() + cs->bandwidth.period);
...
static void update_curr_fair(struct rq* rq) {
  ...
  for (; se; se = se->parent) {
    ...
    if (se->parent) {
      /* Check if this entity's parent group has bandwidth limits */
      struct resdomain* rd = curr->rd;
      struct cpu_rd_state* cs = rd ? (struct cpu_rd_state*)rd->subsys[RD_SUBSYS_CPU] : nullptr;
      if (cs && cs->bandwidth.quota != (uint64_t)-1) {
        spinlock_lock(&cs->bandwidth.lock);
        if (cs->bandwidth.runtime <= delta_exec_ns) {
          cs->bandwidth.runtime = 0;
          /* Note: throttling happens in task_tick or here?
             For simplicity, let's flag for throttling. */
          cfs_rq->throttled = 1;
        } else {
          cs->bandwidth.runtime -= delta_exec_ns;
        }
        spinlock_unlock(&cs->bandwidth.lock);
      }

      if (cs) {
        atomic64_add(delta_exec_ns, &cs->usage);
      }
    }
  }
}
...
static struct task_struct* pick_next_task_fair(struct rq* rq) {
  ...
  if (cfs_rq->throttled) return nullptr;
  ...
  do {
    ...
    if (se->my_q) {
      if (se->my_q->throttled) return nullptr;
      cfs_rq = se->my_q;
      continue;
    }
...
static int select_idle_sibling(struct task_struct* p, int prev_cpu, int target) {
  ...
  for_each_cpu(cpu, &sd->span) {
    ...
    if (this_core_idle && !idle_core_found) {
      ...
      /*
       * Hybrid optimization:
       * If task utilization is very low (< 10%), prefer E-cores (lower capacity)
       * to keep P-cores free for heavy tasks. Otherwise, prefer P-cores.
       */
      unsigned long task_util = p->se.avg.util_avg;
      bool prefer_e = (task_util < 102); /* 102/1024 ~= 10% */
