...
{
	return radix_tree_delete_item(&idr->idr_rt, id - idr->idr_base, nullptr);
}
 ...
 * @id: Pointer ID.
 *
 * Looks up the pointer associated with this ID.  A %nullptr pointer may
 * indicate that @id is not allocated or that the %nullptr pointer was
...
int idr_for_each(const struct idr *idr,
		int (*func)(int id, void *p, void *data), void *data)
{
	struct radix_tree_iter iter;
	void __rcu **slot;
	...
	radix_tree_for_each_slot(slot, &idr->idr_rt, &iter, 0) {
		...
		if (WARN_ON_ONCE(id > INT_MAX))
			break;
		ret = func(id, rcu_dereference_raw(*slot), data);
		if (ret)
			return ret;
	}
			...
			gfp_t gfp)
{
	...
	unsigned long flags;
	struct ida_bitmap *bitmap, *alloc = nullptr;
...
delete:
 ...
 err:
	xas_unlock_irqrestore(&xas, flags);
	printk(KERN_WARNING KERN_CLASS "ida_free called for id=%d which is not allocated.\n", id);
}
...
static void ida_dump_entry(void *entry, unsigned long index)
{
	...
	if (xa_is_node(entry)) {
		...
		printk(KERN_CLASS "value: data %lx [%px]\n", xa_to_value(entry), entry);
	} else {
		struct ida_bitmap *bitmap = entry;

		xa_dump_index(index * IDA_BITMAP_BITS, IDA_CHUNK_SHIFT);
		printk(KERN_CLASS "bitmap: %p data", bitmap);
		for (i = 0; i < IDA_BITMAP_LONGS; i++)
			printk(KERN_CLASS " %lx", bitmap->bitmap[i]);
		printk(KERN_CLASS "\n");
	}
}
