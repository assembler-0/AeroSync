 ...
 * to chaining of merge() calls: nullptr-terminated, no reserved or
...
{
	struct list_head *list = head->next, *pending = nullptr;
	size_t count = 0;	/* Count of pending */

	if (list == head->prev)	/* Zero or one elements */
		return;

	/* Convert to a nullptr-terminated singly-linked list. */
	head->prev->next = nullptr;

	/*
	 * Data structure invariants:
	 * - All lists are singly linked and nullptr-terminated; prev
