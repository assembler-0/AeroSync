...
static void ide_select_drive(struct ide_channel *chan, uint8_t drive) {
  ...
  for (int i = 0; i < 4; i++)
    inb(chan->ctrl_base + ATA_REG_ALT_STATUS);
}
...
static int ide_identify(struct ide_device *ide) {
  ...
  /* Check if it's ATAPI */
  if (inb(chan->io_base + ATA_REG_LBA_MID) != 0 ||
      inb(chan->io_base + ATA_REG_LBA_HIGH) != 0) {
    ide->atapi = true;
...
static int ide_read(struct block_device *bdev, void *buffer,
  ...
  if (ide->atapi) {
    ...
    if (irqs_enabled()) {
      int ret =
          ide_atapi_read_dma(ide, (uint32_t)start_sector, sector_count, buffer);
...
static int ide_write(struct block_device *bdev, const void *buffer,
                     uint64_t start_sector, uint32_t sector_count) {
  struct ide_device *ide = (struct ide_device *)bdev;
...
static int ide_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    /* Allocate PRDT (one page is enough for many entries, but we only need one
     * for now) */
    chan->prdt =
        dma_alloc_coherent(&pdev->dev, PAGE_SIZE, &chan->prdt_phys, GFP_KERNEL);
    ...
    for (int d = 0; d < 2; d++) {
      ...
      if (ide_identify(ide) == 0) {
        ...
        if (ide->atapi) {
          ...
          block_device_assign_name(&ide->bdev, CONFIG_IDE_NAME_PREFIX,
                                   i * 2 + d);
        }
        ...
        if (block_device_register(&ide->bdev) == 0) {
          printk(KERN_INFO ATA_CLASS "Found %s: %s (%llu MB)\n",
                 ide->bdev.dev.name, ide->model,
                 (ide->sectors * ide->bdev.block_size) / 1024 / 1024);
        }
      } else {
        kfree(ide);
      }
...
static void ide_remove(struct pci_dev *pdev) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (chan->prdt)
      dma_free_coherent(&pdev->dev, PAGE_SIZE, chan->prdt, chan->prdt_phys);
  }
}
...
static struct pci_device_id ide_pci_ids[] = {
    {.vendor = PCI_ANY_ID,
     ...
     .class = 0x010100,
     .class_mask = 0xFFFF00}, /* IDE Controller */
    {0}};
...
static int ide_init(void) { return pci_register_driver(&ide_pci_driver); }

static void ide_exit(void) { pci_unregister_driver(&ide_pci_driver); }

asrx_module_init(ide_init);
asrx_module_exit(ide_exit);
asrx_module_info(ide, KSYM_LICENSE_GPL);
