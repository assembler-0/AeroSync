...
static int ide_do_dma(struct ide_device *ide, uint64_t lba, uint32_t count, void *buf, bool is_write) {
  ...
  /* Map buffer for DMA */
  dma_addr_t phys = dma_map_single(buf, count * 512, is_write ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
  ...
  /* Unmap buffer */
  dma_unmap_single(phys, count * 512, is_write ? DMA_TO_DEVICE : DMA_FROM_DEVICE);
