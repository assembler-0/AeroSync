...
static int ide_do_dma(struct ide_device* ide, uint64_t lba, uint32_t count, void* buf, bool is_write) {
  ...
  while (retries--) {
    ...
    bool use_lba48 = ide->lba48 && (lba >= 0x0FFFFFFF || count > 256);

    if (use_lba48) {
      outb(chan->io_base + ATA_REG_DRIVE, 0x40 | (ide->drive << 4));
      outb(chan->io_base + ATA_REG_SEC_COUNT, (uint8_t)(count >> 8));
      outb(chan->io_base + ATA_REG_LBA_LOW, (uint8_t)(lba >> 24));
      outb(chan->io_base + ATA_REG_LBA_MID, (uint8_t)(lba >> 32));
      outb(chan->io_base + ATA_REG_LBA_HIGH, (uint8_t)(lba >> 40));
    ...
    uint8_t ata_cmd;
    if (use_lba48) {
      ata_cmd = is_write ? ATA_CMD_WRITE_DMA_EXT : ATA_CMD_READ_DMA_EXT;
    } else {
      ata_cmd = is_write ? ATA_CMD_WRITE_DMA : ATA_CMD_READ_DMA;
    }
    outb(chan->io_base + ATA_REG_COMMAND, ata_cmd);

    /* Start Bus Master DMA */
    outb(chan->bmide_base + BMIDE_REG_COMMAND, cmd | BMIDE_CMD_START);

    /* Wait for completion (interrupt) with timeout (5s) */
    unsigned long remaining = wait_for_completion_timeout(&chan->done, 5000000000ULL);
    ...
    ret = 0;
    if (remaining == 0) {
      printk(KERN_WARNING "ide: DMA timeout on %s (LBA %llu)\n", ide->bdev.dev.name, lba);
      ret = -ETIMEDOUT;
    } else if (status & BMIDE_STATUS_ERROR || chan->error) {
      ret = -EIO;
    }
    ...
    /* Recovery: Reset channel and try again */
...
int ide_atapi_read_dma(struct ide_device* ide, uint32_t lba, uint32_t count, void* buf) {
  ...
  while (retries--) {
    ...
    while (!((status = inb(chan->io_base + ATA_REG_STATUS)) & ATA_SR_DRQ)) {
      if (status & ATA_SR_ERR || get_time_ns() > timeout) {
        drq_fail = true;
        break;
      }
      cpu_relax();
    ...
    if (remaining == 0) {
      ret = -ETIMEDOUT;
    } else if (bm_status & BMIDE_STATUS_ERROR || chan->error) {
      ret = -EIO;
    }
