...
static int fb_char_open(struct char_device *cdev) {
  (void) cdev;
  return 0;
}

static int fb_char_ioctl(struct char_device *cdev, uint32_t cmd, void *arg) {
  (void) cdev;
  if (!fb) return -ENODEV;

  switch (cmd) {
    case 0x4600: {
      // FBIOGET_VSCREENINFO (simplified)
...
static struct char_operations fb_char_ops = {
  .open = fb_char_open,
  .ioctl = fb_char_ioctl,
};
...
static int linearfb_init(volatile struct limine_framebuffer_request *fb_req) {
  ...
  /* Register using unified Framebuffer interface */
  fb_register_device(&fb_char_ops, nullptr);
...
void linearfb_blit(linearfb_surface_t *dst, linearfb_surface_t *src, uint32_t dx, uint32_t dy, uint32_t sx, uint32_t sy,
...
void linearfb_fill_rect_gradient(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t c1, uint32_t c2,
                                 int vertical) {
  if (!fb) return;
  if (vertical) {
    for (uint32_t i = 0; i < h; ++i) {
