 ...
 *
 * @file arch/x86_64/drivers/apic/x2apic.c
 * @brief x2APIC driver
...
#include <arch/x86_64/drivers/apic/x2apic.h>
...
#define X2APIC_IRR_BASE                                                        \
  0x00000820ULL                  // Interrupt Request Register (8 registers)
...
static void x2apic_write(uint32_t msr, uint64_t value) { wrmsr(msr, value); }

static uint64_t x2apic_read(uint32_t msr) { return rdmsr(msr); }

static void x2apic_write_op(uint32_t msr, uint32_t value) { wrmsr(msr, value); }

static uint32_t x2apic_read_op(uint32_t msr) { return (uint32_t)rdmsr(msr); }

uint32_t x2apic_get_id_raw(void) { return (uint32_t)x2apic_read(X2APIC_ID); }
...
static void x2apic_send_ipi_op(uint32_t dest_apic_id, uint8_t vector,
                               uint32_t delivery_mode) {
  irq_flags_t flags = spinlock_lock_irqsave(&x2apic_ipi_lock);

  uint64_t icr_value = (uint64_t)vector | (uint64_t)delivery_mode |
                       (1ULL << 14) | // Assert
...
static int x2apic_init_lapic(void) {
  ...
  wrmsr(APIC_BASE_MSR,
        lapic_base_msr | APIC_BASE_MSR_ENABLE | APIC_BASE_MSR_X2APIC_ENABLE);
  ...
  if ((version & 0xFF) == 0 || (version & 0xFF) == 0xFF) {
    printk(KERN_ERR APIC_CLASS
           "x2APIC not responding after enable (version: 0x%llx)\n",
           version & 0xFF);
    return 0;
  }
