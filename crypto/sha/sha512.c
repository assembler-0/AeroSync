...
static int crypto_sha512_init(void* ctx) {
  sha512_init(ctx);
  return 0;
}

static int crypto_sha512_update(void* ctx, const uint8_t* data, size_t len) {
  sha512_update(ctx, data, len);
  return 0;
}

static int crypto_sha512_final(void* ctx, uint8_t* out) {
...
static struct crypto_alg sha512_alg = {
  ...
  .ctx_size = sizeof(sha512_context),
...
int __init sha512_generic_init(void) {
  return crypto_register_alg(&sha512_alg);
}
