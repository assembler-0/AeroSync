...
int idr_for_each(const struct idr *, int (*func)(int id, void *p, void *data),
                 void *data);
...
static inline void idr_preload_end(void) {
  spin_unlock(&radix_tree_preloads.lock);
}
...
#define idr_for_each_entry_ul(idr, entry, tmp, id)                             \
  for (tmp = 0, id = 0; ((entry) = tmp <= id ? idr_get_next_ul(idr, &(id))     \
                                             : nullptr) != nullptr;            \
       tmp = id, ++id)
...
#define idr_for_each_entry_continue(idr, entry, id)                            \
  for ((entry) = idr_get_next((idr), &(id)); entry;                            \
       ++id, (entry) = idr_get_next((idr), &(id)))

/**
 * idr_for_each_entry_continue_ul() - Continue iteration over an IDR's elements
 * of a given type
 * @idr: IDR handle.
 * @entry: The type * to use as a cursor.
 * @tmp: A temporary placeholder for ID.
...
#define idr_for_each_entry_continue_ul(idr, entry, tmp, id)                    \
  for (tmp = id; ((entry) = tmp <= id ? idr_get_next_ul(idr, &(id))            \
                                      : nullptr) != nullptr;                   \
       tmp = id, ++id)
