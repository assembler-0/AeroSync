...
#define xa_trylock(xa)		spinlock_trylock(&(xa)->xa_lock)
#define xa_lock(xa)		spinlock_lock(&(xa)->xa_lock)
#define xa_unlock(xa)		spinlock_unlock(&(xa)->xa_lock)
#define xa_lock_bh(xa)		spinlock_lock(&(xa)->xa_lock)
#define xa_unlock_bh(xa)	spinlock_unlock(&(xa)->xa_lock)
#define xa_lock_irq(xa)		spinlock_lock(&(xa)->xa_lock)
#define xa_unlock_irq(xa)	spinlock_unlock(&(xa)->xa_lock)
#define xa_lock_irqsave(xa, flags) \
				spin_lock_irqsave(&(xa)->xa_lock, flags)
#define xa_unlock_irqrestore(xa, flags) \
				spinlock_unlock_irqrestore(&(xa)->xa_lock, flags)
#define xa_lock_nested(xa, subclass) \
				spinlock_lock(&(xa)->xa_lock)
...
int  __xa_insert(struct xarray *, unsigned long index,
		void *entry, gfp_t);
int  __xa_alloc(struct xarray *, uint32_t *id, void *entry,
		struct xa_limit, gfp_t);
int  __xa_alloc_cyclic(struct xarray *, uint32_t *id, void *entry,
		struct xa_limit, uint32_t *next, gfp_t);
 ...
 */
static inline int  xa_insert(struct xarray *xa,
 ...
 * -ENOMEM if memory could not be allocated.
 */
static inline int  xa_insert_bh(struct xarray *xa,
		unsigned long index, void *entry, gfp_t gfp)
{
	int err;
 ...
 * -ENOMEM if memory could not be allocated.
 */
static inline int  xa_insert_irq(struct xarray *xa,
		unsigned long index, void *entry, gfp_t gfp)
{
	int err;
 ...
 */
static inline  int xa_alloc(struct xarray *xa, uint32_t *id,
 ...
 * -EBUSY if there are no free entries in @limit.
 */
static inline int  xa_alloc_bh(struct xarray *xa, uint32_t *id,
		void *entry, struct xa_limit limit, gfp_t gfp)
{
	int err;
 ...
 * -EBUSY if there are no free entries in @limit.
 */
static inline int  xa_alloc_irq(struct xarray *xa, uint32_t *id,
		void *entry, struct xa_limit limit, gfp_t gfp)
{
	int err;
 ...
 */
static inline int xa_alloc_cyclic(struct xarray *xa, uint32_t *id, void *entry,
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_bh(struct xarray *xa, uint32_t *id, void *entry,
		struct xa_limit limit, uint32_t *next, gfp_t gfp)
{
	int err;
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_irq(struct xarray *xa, uint32_t *id, void *entry,
		struct xa_limit limit, uint32_t *next, gfp_t gfp)
{
	int err;
...
{
	return rcu_dereference(xa->xa_head);
}
...
{
	return rcu_dereference(node->parent);
}
