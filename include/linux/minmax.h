...
static inline bool in_range64(uint64_t val, uint64_t start, uint64_t len)
{
	return (val - start) < len;
}

static inline bool in_range32(uint32_t val, uint32_t start, uint32_t len)
{
	return (val - start) < len;
}
...
#define in_range(val, start, len)					\
	((sizeof(start) | sizeof(len) | sizeof(val)) <= sizeof(uint32_t) ?	\
