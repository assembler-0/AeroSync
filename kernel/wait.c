...
extern struct rq per_cpu_runqueues[MAX_CPUS];
...
long __wait_event_timeout(wait_queue_head_t *wq, int condition, long timeout)
{
    ...
    // Convert timeout from jiffies/seconds to nanoseconds (simplified as 1 jiffy = 1 second)
...
/**
 * __wait_event_interruptible_timeout - Wait for a condition with timeout and interrupt support
 * @wq: Wait queue to wait on
 * @condition: Condition to wait for
 * @timeout: Timeout in jiffies (currently treated as seconds for simplicity)
 */
...
void wake_up_interruptible(wait_queue_head_t *wq_head)
{
    ...
    list_for_each(pos, &wq_head->task_list) {
        ...
        if (curr->task && (curr->task->state == TASK_INTERRUPTIBLE || curr->task->state == TASK_RUNNING)) {
            if (curr->func) {
                int ret = curr->func(wq_head, curr, TASK_NORMAL, 0);
                if (ret)
                    nr_woken++;
...
/**
 * interruptible_sleep_on - Put current task to interruptible sleep on a wait queue
 * @wq: Wait queue to sleep on
 */
